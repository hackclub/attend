module Twilio
  class IncomingMessagesController < ActionController::Base
    ...
    def create
      ::Support::ProcessIncomingTwilioMessage.call(params.to_unsafe_h)
      head :ok
    rescue => e
      Rails.logger.error("[Twilio::IncomingMessages] #{e.class}: #{e.message}")
    ...
    def validate_twilio_signature!
      ...
      unless auth_token.present?
        Rails.logger.warn("[Twilio::IncomingMessages] No auth token configured")
        head :forbidden
        return
      end
      ...
      unless validator.validate(url, params_hash, signature)
        Rails.logger.warn("[Twilio::IncomingMessages] Invalid Twilio signature")
        head :forbidden
      end
    end
